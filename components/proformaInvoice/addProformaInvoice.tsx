...
const AddProformaInvoice = ({ ClientList, ServicesList, companyData, invoiceNo, id, mode }: {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            const [res, clientData, servicesData, invoiceNo] = await Promise.all([
                fetchProformaInvoiceById(id),
                fetchClients(),
                fetchServices(),
                proformaInvoiceString()
            ]);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (mode === "new") {
                const res = await insertProformaInvoice(data, items, customTax);
                if (!res.success) {
                    alert(res.message)
                    return;
                }
            } else if (mode === "update" && id) {
                const res = await updateProformaInvoice(id, data, items, customTax);
                if (!res.success) {
                    alert(res.message)
                    return;
                }
                ...
                >
                        <form onSubmit={handleSubmit}>
                            <DialogHeader className="p-6 pb-2">
                                <DialogTitle className="text-xl">Proforma Invoice Details</DialogTitle>
                                <DialogDescription>
                                    Enter the details below to generate a new proforma invoice in the system.
                                </DialogDescription>
                            </DialogHeader>
